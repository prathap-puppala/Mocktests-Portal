...
<div class="row">
   ...
   <table  width="100%" style="text-align:justify;" class="table table-bordered table-striped">
                <tbody>
					<tr><th colspan='2'><center>Answers <b style='color:red;'>After Exam Online </b> mode</center></th></tr>
			</tbody>
			</table>
            <table id="example" class="table display">
				...
				if($teckzitemode)
				{
				$q=mysql_query("SELECT * FROM examdetails WHERE eid!='NULL' and visibility='1' and answersmode='answersafterexamonline' and validations>0");
			    }
			    else
			    {
				$q=mysql_query("SELECT * FROM examdetails WHERE eid='NULL' and visibility='1' and answersmode='answersafterexamonline' and validations>0");
	...
	echo $startt;
                ...
                <td><?php mysql_select_db("quiz_exams",$con); echo mysql_num_rows(mysql_query("SELECT * FROM $tab WHERE status='Y'"));?></td>
                       ...
                       <td><a class="btn btn-rounded btn-option1" name="logic/php/admin/sendmsg.php?sno=<?php echo $w['sno'];?>" onclick="load_page(this.name)" style='cursor:pointer;'>Send</a></td> 
                    </tr>
                    <?php } ?>
                </tbody>
            </table>
