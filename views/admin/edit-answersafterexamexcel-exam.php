...
if($questions!=$puppala['questions'])
{
mysql_select_db($dbname,$con);
...
if($type!=$puppala['type'])
{
mysql_select_db("quiz_exams",$con);
mysql_query("TRUNCATE $number") or die(mysql_error());
}	

if($options!=$puppala['options'])
{
	mysql_select_db("quiz_exams",$con);
...
if(mysql_query("UPDATE examdetails SET eid='$eid',tkey='$number',title='$examtitle',filename='$newf',filetype='$ext',countdown='$countdown',ccone='$ccone',cwone='$cwone',date='$examdate',display='$ExamStartt',endat='$EndExamt',type='$type',options='$options',questions='$questions',addedip='$ip' WHERE sno='$sno'") or die(mysql_error()))		
	{
...
$prathap=mysql_query("SELECT * FROM examdetails WHERE sno='$sno' and answersmode='answersafterexamexcel'");
...
<center>
	...
	echo $H.':'.$M.' '.$m;?>">
                ...
                <tr>
                  ...
                  <div class="col-sm-10">
                     <input type="text" class="form-control" name='questions'  id='ques' value="<?php echo $puppala['questions'];?>"><br>
                     <span id='ans'></span>
                    </div>
                </td>
					</tr>

                     <input type="hidden" class="form-control" name='ccone'  id='ccone' value="1" readonly><br>
                     <input type="hidden" class="form-control" name='cwone'  id='cwone' value="0" readonly><br>
...
if (ques==0 || ques==null || ques>48 || ques==''){alert('Enter Number of Questions\nQuestion Must Be Less than 48.');return false;}
